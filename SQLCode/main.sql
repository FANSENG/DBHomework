...
-- 6.未归还图书视图(租借号、会员名、会员手机号、书名、租借日期、是否续借、租借截止日期)
...
--1.修改续租状态时修改应还日期
--2.修改是否归还状态时修改库存数量、冻结余额
--3.插入租借记录时修改库存数量、冻结余额
--4.插入销售清单时修改全新数量
--5.插入进货订单时修改全新数量
--6.插入充值记录时修改会员信息
--7.插入赔偿记录时修改书籍库存信息
--8.租借图书时检查此会员是否租借过此图书
...
--region 2.修改是否归还状态时修改库存数量、冻结余额、余额
GO
CREATE TRIGGER alterWeatherReturn
on Lease
after update
as
    begin
        ...
        declare @WhetherReturn char(1)

        select @memberID = inserted.memberID,@BookISBN = inserted.BookISBN,@LeaseDay = DATEDIFF(day,inserted.LeaseDate,GETDATE()),@WhetherReturn = inserted.WhetherReturn from inserted
        select @BookPrice = BookPrice
        from BookData where BookISBN = @BookISBN

        if @WhetherReturn=N'是'
        begin
            update BookData
...
as
    begin
        ...
        select @memberID = inserted.memberID,@BookISBN = inserted.BookISBN from inserted
        select @BookPrice = BookPrice
        from BookData where BookISBN = @BookISBN
...
as
    begin
        ...
        select @memberID = inserted.memberID,@BookISBN = inserted.BookISBN,@CompensationMoney = inserted.CompensateMoney from inserted
        ...
        and Lease.BookISBN = @BookISBN
        and WhetherReturn = N'否'
    end
--endregion

--region 8.租借图书时检查此会员是否租借过此图书
Go
CREATE TRIGGER insertLeaseDataCheck
on Lease
after INSERT
as
    begin
        ...
        if exists (select * from Lease where Lease.BookISBN = @BookISBN and Lease.memberID = @memberID and Lease.WhetherReturn = N'否')
            begin
